...
const Chat = () => {
  ...
  const sendEvent = async (data) => {
    ...
    const graphqlMutation = {
      operationName: "createChatEvent",
      query: `
        mutation createChatEvent($input: ChatEventCreateInput!) {
          event: createChatEvent(input: $input) {
              status,
              data {
                  id
              },
      ...
      variables: {
        input: {
            roomIdentifier: config.CHAT_ROOM_ID,
            eventName: data.eventName,
            attributes: data.eventAttributes
        }
      }
    };

    console.info(`Event attributes: ${data.eventAttributes}`);

    try {
      const response = await axios.post(
        // `${config.API_URL}/graphql`,
        "https://6a0wopldul.execute-api.us-west-2.amazonaws.com/graphql",
        graphqlMutation,
        graphqlConfig
      );
